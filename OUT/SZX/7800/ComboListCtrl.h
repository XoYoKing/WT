...
#define WM_GET_ONFOCUSITEM	(WM_USER + 0x7F10)

#define COMBOLIST                     1
#define EDITLIST                      2
#define DATETIMELIST                  3

typedef struct STRU_LISTCTRLONFOCUS
{
	HANDLE hControl;                          //控件句柄
	BYTE bControlType;                        //控件类型
	int iRow;                                 //表行号
	int iColumn;                              //表列号

	union
	{
		int iComboSelCur;                     //下拉框控件焦点索引
		char cEditContext[256];     //编辑框控件当前内容
		time_t tDateTime;                     //时间控件当前时间
	};

	STRU_LISTCTRLONFOCUS()
	{
		hControl = NULL;
		iRow = -1;
		iColumn = -1;
...
protected:
	virtual LRESULT WindowProc(UINT message, WPARAM wParam, LPARAM lParam); // yxn20090819
public:
};
